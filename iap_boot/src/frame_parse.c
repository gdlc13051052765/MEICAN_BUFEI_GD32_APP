...
uint8_t msg_frame_check(void *instance)
{
	...
	if((pthis->fw_msg.MsgFrame.FrameStep == 0x00) && (pthis->fw_msg.MsgFrame.AppTotalPackage > 10) && (crc16_ccitt(pthis->fw_msg.FrameBuff, FRAME_LENGTH - 2) == pthis->fw_msg.MsgFrame.CRC16))
	{
		return 0x01;
	}
	else
...
uint8_t firmware_data_check(void *instance, uint16_t *fw_id)
{
	...
	while(cur_pkg_step < pthis->fw_msg.MsgFrame.AppTotalPackage)
	{
		...
		if(pthis->r_flash->read(read_fw_addr, fw_cache, FRAME_LENGTH))
		{
           ...
           fw_src_crc32 = CRC32DataCheckFun((fw_cache + 4), (FRAME_LENGTH));
			}
			else
			{
				*fw_id = cur_pkg_step;
...
uint8_t find_firmware_lost_step(void *instance, uint16_t *fw_id)
{
	...
	printf("pthis->fw_msg.MsgFrame.AppTotalPackage：%d\r\n", pthis->fw_msg.MsgFrame.AppTotalPackage);
	...
	do{
		...
		if(pthis->r_flash->read(read_fw_addr, fw_cache, FRAME_LENGTH))
		{
			...
			if(pmsg->DataFrame.CRC16 == crc16_ccitt(fw_cache, FRAME_LENGTH - 2))   //校验数据帧的完整性
			{
				...
				if(pthis->fw_msg.MsgFrame.AppVer != pmsg->DataFrame.SoftVer)
				{
					printf("校验版本 不能使用读取的数据");
					*fw_id = cur_pkg_step; //不能使用读取的数据
					return ret_need_pkg;
				}
				
				//校验帧区分（LSF W2E锁控板的升级协议里的数据包里的帧区分与版型不一致，所以不做判断）
//				if(pthis->fw_msg.MsgFrame.board_type != pmsg->DataFrame.FrameType)
//				{
//					*fw_id = cur_pkg_step; //不能使用读取的数据
//					return ret_need_pkg;
...
//				}
    ...
    pthis->r_flash->read(pthis->boot_msg_addr, (uint8_t*)p_IAP_Status, FRAME_LENGTH);
    
    IAP_Status.magicNumber = TO_IAP_FLAG;
    IAP_Status.appAddress = APP_START_ADDR;
    IAP_Status.firmwareByteWritten = pthis->fw_msg.MsgFrame.AppTotalPackage;
    IAP_Status.firmwareSize = IAP_Status.firmwareByteWritten * FRAME_LENGTH;   
    pthis->r_flash->write(pthis->boot_msg_addr, (uint8_t*)p_IAP_Status, FRAME_LENGTH);

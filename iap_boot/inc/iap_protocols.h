...
typedef struct
{
	IAP_StatusEnum status;                  //升级状态 
	
	uint32_t magicNumber;
	
	uint32_t infoAddress;                   //存储信息的地址
	uint32_t firmwareDataAddress;           //代码在FLASH 中的存储位置 
	uint32_t appAddress;                    //app 在片上地址
	
	uint32_t firmwareSize;                  //总大小
	uint32_t Crc32;

	uint32_t firmwareByteWritten;           //已经写的数量(字节数)
	uint32_t firmwareCurrentWritePtr;       //已经写的当前地址

	uint32_t Key32;
	uint32_t Update_Times;                  //升级次数

}IAP_StatusTypeDef;

extern IAP_StatusTypeDef IAP_Status;
extern IAP_StatusTypeDef *p_IAP_Status;
